...
def create(config):
	...
	for i in xrange(dim_d):
		model['Wc_%i' %i] = tf.Variable(tf.random_uniform([cwidth, cwidth, 1, 1], -np.sqrt(3. / (cwidth)), np.sqrt(3. / (cwidth))), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'Wc_%i' %i)
		...
		model['Wf_%i' %i] = tf.Variable(tf.random_uniform([dim_i - dim_f, dim_i - dim_f], -np.sqrt(3. / (dim_i - dim_f)), np.sqrt(3. / (dim_i - dim_f))), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'Wf_%i' %i)
	...
	model['W_%i' %dim_d] = tf.Variable(tf.random_uniform([dim_x * dim_y + dim_i - dim_f, dim_o], -np.sqrt(6. / (dim_x * dim_y + dim_i - dim_f + dim_o)), np.sqrt(6. / (dim_x * dim_y + dim_i - dim_f + dim_o))), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'W_%i' %dim_d)
	model['b_%i' %dim_d] = tf.Variable(tf.random_uniform([1, dim_o], -np.sqrt(6. / dim_o), np.sqrt(6. / dim_o)), name = 'b_%i' %dim_d)
	model['x_%i' %dim_d] = tf.concat(1, [tf.reshape(tf.squeeze(model['yc_%i' %(dim_d - 1)], [-1]), [dim_b, dim_x * dim_y]), model['yf_%i' %(dim_d - 1)]], name = 'xc_%i' %dim_d)
